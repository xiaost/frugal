	...
	testcases := []testcase{
		...
		},
		{
			...
			update: func(p0 *TestTypes) {
				p0.M0 = map[int32]int32{vInt32: vInt32 - 1}
				p0.M1 = map[int32]string{vInt32 - 2: "hello"}
				p0.M2 = map[int32]*Msg{vInt32 - 3: nil}
				p0.M3 = map[string]*Msg{"hello": &Msg{Type: vInt32 - 4}}
			},
			test: func(t *testing.T, p1 *TestTypes) {
				assert.Equal(t, map[int32]int32{vInt32: vInt32 - 1}, p1.M0)
				assert.Equal(t, map[int32]string{vInt32 - 2: "hello"}, p1.M1)
				assert.Equal(t, map[int32]*Msg{vInt32 - 3: &Msg{}}, p1.M2)
		...
		},
		{
			...
			test: func(t *testing.T, p1 *TestTypes) {
				assert.Equal(t, []int32{1}, p1.L0)
				assert.Equal(t, []string{"hello"}, p1.L1)
		...
		},
		{
			...
			test: func(t *testing.T, p1 *TestTypes) {
				assert.Equal(t, []int32{1}, p1.S0)
				assert.Equal(t, []string{"hello"}, p1.S1)
			},
		},
	}
...
func TestDecodeRequired(t *testing.T) {
	...
	p1 := &S1{}
	_, err = Decode(b, p1)
	require.Equal(t, &requiredFieldNotSetException{message: `required field "V" is not set`}, err)
