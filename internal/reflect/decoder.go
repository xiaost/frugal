...
var btype = unsafe.Pointer(rvTypePtr(reflect.ValueOf(byte(0))))

type span struct {
	typ unsafe.Pointer // nil for noscan, or btype
...
func (s *span) init() {
	...
	s.p = 0
	s.b = mallocgc(uintptr(sz), s.typ, s.typ != nil) //  s.typ == nil -> noscan, nozero
	s.n = sz
}
...
func (s *span) Malloc(n, align int) unsafe.Pointer {
	...
	if s.p+n+mask > s.n {
		...
		s.p = 0
		s.b = mallocgc(uintptr(sz), s.typ, s.typ != nil)
		s.n = sz
	}
...
func (d *Decoder) Malloc(n, align int, noscan bool) unsafe.Pointer {
	if n > defaultDecoderMemSize/8 {
		if noscan {
			return mallocgc(uintptr(n), nil, false)
		}
		return mallocgc(uintptr(n), btype, true)
	}
	if noscan {
		return d.s1.Malloc(n, align)
	}
	return d.s0.Malloc(n, align)
}

func (d *Decoder) mallocIfPointer(t *tType, p unsafe.Pointer) (ret unsafe.Pointer) {
	if t.IsPointer {
		// we need to malloc the type first before assigning a value to it
		ret = d.Malloc(t.V.Size, t.V.Align, false)
...
func (d *Decoder) Decode(b []byte, base unsafe.Pointer, fd *FieldDesc, maxdepth int) (int, error) {
	if maxdepth == 0 {
...
func (d *Decoder) decodeType(t *tType, b []byte, p unsafe.Pointer, maxdepth int) (int, error) {
	if maxdepth == 0 {
		return 0, errDepthLimitExceeded
	}
	if t.FixedSize > 0 {
	...
	case tMAP:
		...
		*((*uintptr)(p)) = m.Pointer() // p = make(t.RT, l)
		...
		if kt.IsPointer {
			sliceK = d.Malloc(l*kt.V.Size, kt.V.Align, kt.V.FixedSize > 0)
		}
		var sliceV unsafe.Pointer
		if vt.IsPointer {
			sliceV = d.Malloc(l*vt.V.Size, vt.V.Align, vt.V.FixedSize > 0)
		}
		...
		for j := 0; j < l; j++ {
			...
			if sliceK != nil {
				*(*unsafe.Pointer)(p) = sliceK
				p = sliceK
				if j != l-1 { // protect pointing to invaild data
					sliceK = unsafe.Add(sliceK, kt.V.Size)
				}
			} else {
				p = d.mallocIfPointer(kt, p)
			}
			if kt.FixedSize > 0 {
	...
	case tLIST, tSET: // NOTE: for tSET, it may be map in the future
		...
		x := d.Malloc(l*et.Size, et.Align, et.FixedSize > 0) // malloc for slice. make([]Type, l, l)
		h.Data = x
