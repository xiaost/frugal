package reflect

import (
	"fmt"
	"reflect"
	"sync"
	"unsafe"
)

// copyn copies n bytes from src to dst addr.
// it mainly used by decoder of tSTRING
func copyn(dst unsafe.Pointer, src []byte, n int) {
	var b []byte
	hdr := (*reflect.SliceHeader)(unsafe.Pointer(&b))
	hdr.Data = uintptr(dst)
	hdr.Cap = n
	hdr.Len = n
	copy(b, src)
}

type fieldBitset struct {
	data [1024]uint64 // 635536 bits for field id
}

var bitsetPool = sync.Pool{
	New: func() interface{} {
		return &fieldBitset{}
	},
}

func (s *fieldBitset) set(i uint16) {
	x, y := i>>8, i&7 // i/64, i%64
	s.data[x] |= 1 << y
}

func (s *fieldBitset) unset(i uint16) {
	x, y := i>>8, i&7 // i/64, i%64
	s.data[x] &= ^(1 << y)
}

func (s *fieldBitset) test(i uint16) bool {
	x, y := i>>8, i&7 //  i/64, i%64
	return (s.data[x] & (1 << y)) != 0
}

// only be used when NewRequiredFieldNotSetException
func lookupFieldName(rt reflect.Type, offset uintptr) string {
	for rt.Kind() == reflect.Pointer {
		rt = rt.Elem()
	}
	for i := 0; i < rt.NumField(); i++ {
		f := rt.Field(i)
		if f.Offset == offset {
			return f.Name
		}
	}
	return "unknown"
}

func checkUniqueness(t *tType, p unsafe.Pointer, h *reflect.StringHeader) error {
	et := t.V
	uniq := true
	switch et.T {
	case tBOOL:
		var vv []bool
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessBool(vv)
	case tI08:
		var vv []int8
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessInt8(vv)
	case tI16:
		var vv []int16
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessInt16(vv)
	case tI32:
		var vv []int32
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessInt32(vv)
	case tI64, tENUM:
		var vv []int64
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessInt64(vv)
	case tDOUBLE:
		var vv []float64
		*(*reflect.StringHeader)(unsafe.Pointer(&vv)) = *h
		uniq = checkUniquenessFloat64(vv)
	case tSTRING:
		var ss []string
		*(*reflect.StringHeader)(unsafe.Pointer(&ss)) = *h
		uniq = checkUniquenessString(ss)
	default: // slow path, TODO: testcases
		p0 := p
	LoopCheckUniqueness:
		for i := 0; i < h.Len; i++ {
			v0 := reflect.NewAt(et.RT, p0)
			p1 := unsafe.Pointer(uintptr(p0) + uintptr(et.Size))
			for j := i + 1; j < h.Len; j++ {
				v1 := reflect.NewAt(et.RT, p1)
				if v0.Equal(v1) {
					uniq = false
					break LoopCheckUniqueness
				}
				p1 = unsafe.Pointer(uintptr(p1) + uintptr(et.Size))
			}
			p0 = unsafe.Pointer(uintptr(p0) + uintptr(et.Size))
		}
	}
	if !uniq {
		return fmt.Errorf("%s error writing set field: slice is not unique", et.RT.String())
	}
	return nil
}

// XXX: checkUniqueness funcs use generics when we start to use newer go versions.
func checkUniquenessBool(vv []bool) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessInt8(vv []int8) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessInt16(vv []int16) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessInt32(vv []int32) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessInt64(vv []int64) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessFloat64(vv []float64) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

func checkUniquenessString(vv []string) bool {
	l := len(vv)
	for i := 0; i < l; i++ {
		for j := i + 1; j < l; j++ {
			if vv[i] == vv[j] {
				return false
			}
		}
	}
	return true
}

type tmpMapVars struct {
	k  reflect.Value  // t.K.RT
	kp unsafe.Pointer // *t.K.RT

	v  reflect.Value  // t.V.RT
	vp unsafe.Pointer // *t.V.RT
}

func initOrGetMapTmpVarsPool(t *tType) *sync.Pool {
	if t.T != tMAP {
		return nil
	}
	return &sync.Pool{
		New: func() interface{} {
			m := &tmpMapVars{}
			m.k = reflect.New(t.K.RT)
			m.kp = m.k.UnsafePointer()
			m.k = m.k.Elem() // make m.k addressable with type t.K.RT
			m.v = reflect.New(t.V.RT)
			m.vp = m.v.UnsafePointer()
			m.v = m.v.Elem() // make m.v addressable with type t.V.RT
			return m
		},
	}
}
